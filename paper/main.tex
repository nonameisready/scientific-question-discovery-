...
\title{Evidence-Based Scientific Question Discovery:\\
\large A Framework with Historical Backtesting}

% Double-blind switch: build the anonymized version with
%   pdflatex -jobname=main_anon "\def\ANONYMOUS{1}\input{main}"
\ifdefined\ANONYMOUS
  \author{Anonymous author(s)\\Double-blind submission}
\else
  \author{%
    Hui Mao\\
    Aurora AI Ventures
...
\begin{abstract}
Current AI systems are optimized for answering questions; the scientific
enterprise is bottlenecked earlier, at \emph{discovering the questions
worth investigating}. We present a framework that turns a
